...
static void setubv_pseudo_arclength(integer ndim,
#ifdef MANIFOLD
       integer nalc,
#endif
       integer ncb, integer nrc, iap_type *iap,
       doublereal *rds, doublereal **dd, doublereal *fc, doublereal *rlcur,
       doublereal *rlold, doublereal *rldot, doublereal **udotps,
       ...
       doublereal **dups, doublereal *dtm, doublereal *thl, doublereal *thu)
{
  ...
  fc[nrc-1] = *rds - rinpr(iap, ndim, udotps, dups, dtm, thu) - rlsum;
#else
  integer m;
  for (m = 0; m < nalc; ++m) {
      for (i = 0; i < ncb; ++i) {
        dd[nrc-nalc+m][i] = thl[i] * rldot[i+m*NPARX];
      }
    rlsum = 0.;
    for (i = 0; i < ncb; ++i) {
      rlsum += thl[i] * (rlcur[i] - rlold[i]) * rldot[i+m*NPARX];
    }
    fc[nrc-nalc+m] = rds[m] - rinpr(iap, ndim, udotps+m*(iap->ntst + 1), dups, dtm, thu) - rlsum;
   }
#endif
}
