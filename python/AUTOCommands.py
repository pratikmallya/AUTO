...
class commandUserData(command):
	...
	rval=valueSystem()
        ...
        rval.info("(Required files : %s.c, c.%s, %s.dat)\n"%(self.data[0],
                                                                     self.data[0],
                                                                     self.data[0]))
        rval.system("cc -I$AUTO_DIR/include -L$AUTO_DIR/lib $AUTO_DIR/src/fcon.c %s.c -o fcon -lauto_f2c -lm"%(self.data[0],))
...
class commandWithFilenameTemplate(command):
    def __init__(self,name1=None,name2=None,templates=None):
        if templates is None:
            ...
            if glob.glob("c.*") == [] and glob.glob("r.*") != []:
                self.templates["constants"]          = "r.%s"
                self.templates["bifurcationDiagram"] = "p.%s"
                self.templates["solution"]           = "q.%s"
                self.templates["homcont"]            = "s.%s"
        else:
...
class commandRelabel(commandWithFilenameTemplate):
	...
	rval=valueSystem()
        if self.name2["bifurcationDiagram"] is None:
            ...
            command = os.path.join(os.environ["AUTO_DIR"],"bin/autlab")
