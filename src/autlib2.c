  ...
  */
  ...
  if (ifst == 1) {
...
#endif
    setubv_make_boundary(ndim, ntst, nbc, nfpr, nrow, bcni, iap, rap, par, icp,
	   main_auto_storage.ccbc, main_auto_storage.d, fc, rlcur, rlold,
	   ups, uoldps, dups);
    setubv(ndim, ips, ntst, ncol, nint,
#ifdef MANIFOLD
	   nalc,
#endif
	   nfpr, nrc-nbc, nrow, nclm, funi, icni, ndxloc, iap, rap, par, icp, 
	   rds, main_auto_storage.a, main_auto_storage.b, main_auto_storage.c, &main_auto_storage.d[nbc], fa, &fc[nbc], rlcur, 
    ...
    setrhs(ndim, ips, ntst, ntst0, main_auto_storage.np, ncol, nbc, nint,
	   ...
	   nalc,
#endif
	   nfpr, nrc, nrow, &iam, &kwt, &ipar, funi, bcni, icni,
	   ndxloc, iap, rap, par, icp, rds, fa, fc, rlcur, 
	   rlold, rldot, ups, uoldps, udotps, upoldp, dups, dtm, thl, thu);
  }
  ...
  print_jacobian(*iap,main_auto_storage);
  {
    ...
    num_calls++;
    print_fa_fc(*iap,fa,fc,filename);
  }
#endif

  brbd(main_auto_storage.a, main_auto_storage.b, main_auto_storage.c, main_auto_storage.d,
       fa, fc, p0, p1, ifst, iid, nllv, &det, ndim, ntst, nbc, nrow, nclm, nfpr,
       nrc, iam, kwt, ipar, main_auto_storage.a1, main_auto_storage.a2, main_auto_storage.bb, 
       main_auto_storage.cc, main_auto_storage.ccbc, main_auto_storage.faa, main_auto_storage.ca1, main_auto_storage.s1, main_auto_storage.s2, 
...
/*     ---------- ------ */
/* Subroutine */ int 
setrhs(integer ndim, integer ips, integer na, integer ntst, integer *np, integer ncol, integer nbc, integer nint,
#ifdef MANIFOLD
       integer nalc,
#endif
...
/*      Partition the mesh intervals. */
  ...
  for (jj = 0; jj < na; ++jj) {
	...
	prm[i] = par[i];
      ...
      ic1 = ic * ndim;
      for (i = 0; i < ndim; ++i) {
	fa[jj][ic1 + i] = f[i] - wploc[ncol][ic] * ups[jp1][i];
	for (k = 0; k < ncol; ++k) {
	  k1 = k * ndim + i;
	  fa[jj][ic1 + i] -= wploc[k][ic] * ups[j][k1];
	}
      }
      /* L1: */
    }
...
/*     Boundary conditions : */
  ...
  rlsum = 0.;
  for (i = 0; i < ncb; ++i) {
    rlsum += thl[i] * (rlcur[i] - rlold[i]) * rldot[i];
  }

  fc[-1 + nrc] = *rds - rinpr(iap, ndim, udotps, 
			       dups, dtm, thu) - rlsum;
#else
  ...
  for(m=0;m<nalc;m++){
    rlsum = 0.;
    for (i = 0; i < ncb; ++i) {
      rlsum += thl[i] * (rlcur[i] - rlold[i]) * rldot[i+m*NPARX];
    }

    fc[nrc-nalc+m] = rds[m] - rinpr(iap, ndim, udotps+m*(iap->ntst + 1), 
			         dups, dtm, thu) - rlsum;
   }
#endif
...
brbd(doublereal ***a, doublereal ***b, doublereal ***c, doublereal **d, doublereal **fa,
     doublereal *fc, doublereal **p0, doublereal **p1, integer ifst, integer idb,
     integer nllv, doublereal *det, integer nov, integer na, integer nbc, integer nra,
     integer nca, integer ncb, integer nrc, integer iam, integer kwt, logical par,
     doublereal ***a1, doublereal ***a2, doublereal ***bb, doublereal ***cc,
     doublereal ***ccbc,
     doublereal **faa, doublereal ***ca1, doublereal ***s1, doublereal ***s2,
     integer *icf11, integer *ipr, integer *icf1, integer *icf2, integer *irf,
     integer *icf
#ifdef MANIFOLD
     ,integer useDefaultRHS,doublereal* RHSVector
#endif
...
{
  doublereal *fcc;
  doublereal *sol1,*sol2;

  if (idb > 4 && iam == 0) {
    print1(na, nra, nca, ncb, nrc, nbc, a, b, c, ccbc, d, fa, fc);
  }
  if (ifst == 1) {
#ifdef USAGE
...
#endif
    conpar(nov, na, nra, nca, a, ncb, b, nrc-nbc, c, &d[nbc], irf, icf);
#ifdef USAGE
    usage_end(conpar_usage,"all of conpar");
#endif
    copycp(na, nov, nra, nca, a, ncb, b, nrc-nbc, c, 
	   a1, a2, bb, cc, irf);
  }

  if (nllv == 0) {
    conrhs(nov, na, nra, nca, a, nrc-nbc, c, fa, &fc[nbc], irf, icf);
  } else {
...
#endif
    reduce(iam, kwt, par, a1, a2, bb, cc, &d[nbc], na, 
	   nov, ncb, nrc-nbc, s1, s2, ca1, icf1, icf2, 
	   icf11, ipr);
#ifdef USAGE
  ...
  if (nllv == 0) {
    redrhs(iam, kwt, par, a1, a2, cc, faa, &fc[nbc], na, 
	   nov, nrc-nbc, ca1, icf1, icf2, icf11, ipr);
  }

  fcc = (doublereal *)malloc(sizeof(doublereal)*((nov + nrc) + (2*nov*nov)+1));

  dimrge(iam, kwt, par, cc, ccbc, d, fc, na, 
	 nrc, nbc, nov, ncb, idb, nllv, fcc, p0, p1, det, s1, a2,
	 faa, bb
#ifdef MANIFOLD
	 ,useDefaultRHS,RHSVector
#endif
  ...
  bcksub(iam, kwt, par, s1, s2, a2, bb, faa, fc, 
	 fcc, sol1, sol2, na, nov, ncb, icf2);

  infpar(a, b, fa, sol1, sol2, fc, na, nov, nra, nca, ncb, irf, icf);
...
/*     ---------- ------ */
/* Subroutine */ int 
conrhs(integer nov, integer na, integer nra, integer nca, doublereal ***a, integer nrc, doublereal ***c, doublereal **fa, doublereal *fc, integer *irf, integer *icf)
{
  /* System generated locals */
  integer icf_dim1, irf_dim1;

    /* Local variables */
  integer i, icfic, irfir, m1, m2, ic, ir, irfirp, ir1, nex, irp;
  ...
  for (i = 0; i < na; ++i) {
	...
	if (a[i][irfir][icfic] != (double)0.) {
	  fa[i][irfir] -= a[i][irfir][icfic] * fa[i][irfirp];
	}
      }
      for (ir = 0; ir < nrc; ++ir) {
	if (c[i][ir][icfic] != (double)0.) {
	  fc[ir] -= c[i][ir][icfic] * fa[i][irfirp];
	}
      }
    }
  }
...
/* Copies the condensed sytem generated by CONPAR into workspace. */
  ...
  for (i = 0; i < na; ++i) {
    for (ir = 0; ir < nov; ++ir) {
      ...
      for (ic = 0; ic < nov; ++ic) {
	ic1 = nca - nov + ic;
	a1[i][ir][ic] = a[i][irfir][ic];
	a2[i][ir][ic] = a[i][irfir][ic1];
      }
      for (ic = 0; ic < ncb; ++ic) {
	bb[i][ir][ic] = b[i][irfir][ic];
      }
    }
  }
...
/*     ---------- ------ */
/* Subroutine */ int 
redrhs(integer iam, integer kwt, logical par, doublereal ***a1, doublereal ***a2, doublereal ***cc, doublereal **faa, doublereal *fc, integer na, integer nov, integer nrc, doublereal ***ca1, integer *icf1, integer *icf2, integer *icf11, integer *ipr)
{
  /* System generated locals */
  ...
  doublereal tmp;
  logical notsend;
  integer nam1, myright[KREDO], icp1;
...
/* at distance 2**(K-1) from the master. Here K is the recursion level. */
  ...
  for (i1 = 0; i1 < nam1; ++i1) {
	...
	l1 = ARRAY2D(icf2, ic, i1);
	rm = a2[i1][ir][l1];
	faa[i1][ir] -= rm * faa[i1][ic];
      }
      for (ir = 0; ir < nov; ++ir) {
	l1 = ARRAY2D(icf1, ic, i2);
	rm = a1[i2][ir][l1];
	faa[i2][ir] -= rm * faa[i1][ic];
      }
      for (ir = 0; ir < nrc; ++ir) {
	l1 = ARRAY2D(icf2, ic, i1);
	rm = cc[i2][ir][l1];
  ...
  if (par) {
	...
	if (master[i]) {
	  ...
	  if (ipiv1 < nov) {
	    buf[0] = faa[na - 1][ipiv1];
	    faa[na][ipiv1] = faa[na - 1][ic];
	    faa[na - 1][ic] = buf[0];
	    buf[1] = -1.;
	    csend();
	  } else {
	  ...
	  for (ir = icp1; ir < nov; ++ir) {
	    l1 = ARRAY2D(icf2, ic, (na - 1));
	    rm = a2[na - 1][ir][l1];
	    faa[na - 1][ir] -= rm * faa[na - 1][ic];
	  }
	  for (ir = 0; ir < nrc; ++ir) {
	    l1 = ARRAY2D(icf2, ic, (na - 1));
	    rm = cc[na][ir][l1];
	    fc[ir] -= rm * faa[na - 1][ic];
	  }
	}
	...
	if (worker[i]) {
	  ...
	  if (ipiv2 < 0) {
	    ...
	    tmp = faa[na - 1][ipiv2];
	    faa[na - 1][ipiv2] = buf[0];
	    csend();
	  }

	  for (ir = 0; ir < nov; ++ir) {
	    l1 = ARRAY2D(icf11, ic, i);
	    rm = ca1[i][ir][l1];
	    faa[na - 1][ir] -= rm * tmp;
	  }
	}
...
/*     ---------- ------ */
/* Subroutine */ int 
dimrge(integer iam, integer kwt, logical par, doublereal ***cc, doublereal ***ccbc, doublereal **d, doublereal *fc, integer na, integer nrc, integer nbc, integer nov, integer ncb, integer idb, integer nllv, doublereal *fcc, doublereal **p0, doublereal **p1, doublereal *det, doublereal ***s, doublereal ***a2, doublereal **faa, doublereal ***bb
#ifdef MANIFOLD
,integer useDefaultRHS,doublereal *RHSVector
#else
#define useDefaultRHS 1
#define RHSVector NULL
#endif
)
{
  ...
  integer novpj2, kc, kr, ncrloc, msglen1, msglen2;
  ...
  if (iam == kwt - 1) {
	...
	novpj = nov + j;
	if (i < nbc) {
	  e[novpi][j] = ccbc[0][i][j];
	  e[novpi][novpj] = ccbc[1][i][j];
	} else {
	  e[novpi][j] = cc[0][i-nbc][j];
	  e[novpi][novpj] = cc[na][i-nbc][j];
	}
      }
      for (j = 0; j < ncb; ++j) {
	novpj2 = (nov * 2) + j;
    ...
    if (nllv == 0) {
      ...
      nlvc(ncrloc, ncrloc, nllv, useDefaultRHS, RHSVector, e, fcc);
    } else{
...
/*     ---------- ------ */
/* Subroutine */ int 
bcksub(integer iam, integer kwt, logical par, doublereal ***s1, doublereal ***s2, doublereal ***a2, doublereal ***bb, doublereal **faa, doublereal *fc, doublereal *fcc, doublereal *sol1, doublereal *sol2, integer na, integer nov, integer ncb, integer *icf2)
{
  /* System generated locals */
  ...
  double *buf=NULL;
  doublereal *sol3 = (doublereal *)malloc(sizeof(doublereal)*nov*(na + 1));
...
*/
  ...
  if (par) {
	...
	for (k = nov - 1; k >= 0; --k) {
	  ...
	  for (l = kp1; l < nov; ++l) {
	    l1 = ARRAY2D(icf2, l, (na - 1));
	    sm += ARRAY2D(sol2, l1, (na - 1)) * a2[na - 1][k][l1];
	  }
	  l2 = ARRAY2D(icf2, k, (na - 1));
	  ARRAY2D(sol2, l2, (na - 1)) = (faa[na - 1][k] - sm) / a2[na - 1][k][l2];
	}
  ...
  for (i = nam1 - 1; i >= 0; --i) {
	...
	sm += ARRAY2D(sol2, l1, i) * a2[i][k][l1];
      ...
      l2 = ARRAY2D(icf2, k, i);
      ARRAY2D(sol2, l2, i) = (faa[i][k] - sm) / a2[i][k][l2];
    }
    for (l = 0; l < nov; ++l) {
...
/*     ---------- ------ */
/* Subroutine */ int 
infpar(doublereal ***a, doublereal ***b, doublereal **fa, doublereal *sol1, doublereal *sol2, doublereal *fc, integer na, integer nov, integer nra, integer nca, integer ncb, integer *irf, integer *icf)
{
  /* System generated locals */
...
*/
  for (i = 0; i < na; ++i) {
	...
	sm += b[i][irfir][j] * fc[novpj];
      ...
      for (j = irp1; j < nram; ++j) {
	j1 = j + nov;
	icfj1 = ARRAY2D(icf, j1, i);
	sm += a[i][irfir][icfj1] * x[icfj1];
      }
      novpir = nov + ir;
      icfnovpir = ARRAY2D(icf, novpir, i);
      x[icfnovpir] = (fa[i][irfir] - sm) / a[i][irfir][icfnovpir];
    }
    /*        **Copy SOL1 and X into FA */
    for (j = 0; j < nov; ++j) {
      fa[i][j] = ARRAY2D(sol1, j, i);
    }
    for (j = nov; j < nra; ++j) {
      fa[i][j] = x[j];
    }
...
/*     ---------- ------ */
/* Subroutine */ int 
print1(integer na, integer nra, integer nca, integer ncb, integer nrc, integer nbc, doublereal ***a, doublereal ***b, doublereal ***c, doublereal ***ccbc, doublereal **d, doublereal **fa, doublereal *fc)
{
  ...
  for (i = 0; i < na; ++i) {
	...
	if((total_written != 0) && (total_written%12 == 0))
	  fprintf(fp9,"\n");
	if (ir >= nbc)
	  fprintf(fp9," %10.3E",c[i][ir-nbc][ic]);
	else if (i == 0 && ic < nca-nra)
	  fprintf(fp9," %10.3E",ccbc[0][ir][ic]);
	else if (i == na-1 && ic >= nra)
	  fprintf(fp9," %10.3E",ccbc[1][ir][ic-nra]);
	else
	  fprintf(fp9," %10.3E",0.0);
	total_written++;
      }
      fprintf(fp9,"\n");	
    }
