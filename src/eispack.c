...
/* ----------------------------------------------------------------------- */
/* Subroutine */ int 
hqr(integer nm, integer n, integer *low, integer *igh, doublereal *h, doublereal *wr, doublereal *wi, integer *ierr)
{
    /* System generated locals */
    ...
    doublereal p = 0, q = 0, r__ = 0, s, t, w, x, y;
    integer na, en, ll, mm;
    doublereal zz;
    logical notlas;
...
/*                AND COMPUTE MATRIX NORM .......... */
	...
	for (j = k; j <= i__2; ++j) {
/* L40: */
	    norm += fabs(h[h_offset + i__ + j * h_dim1]);
	}
	...
	wr[i__] = h[h_offset + i__ + i__ * h_dim1];
	wi[i__] = 0.;
L50:
	;
    }
...
L70:
	...
	s = fabs(h[h_offset + l - 1 + (l - 1) * h_dim1]) + fabs(h[h_offset + l 
		+ l * h_dim1]);
	if (s == 0.) {
	    s = norm;
	}
	tst1 = s;
	tst2 = tst1 + fabs(h[h_offset + l + (l - 1) * h_dim1]);
	if (tst2 == tst1) {
	    goto L100;
	}
/* L80: */
...
/*     .......... FORM SHIFT .......... */
L100:
    x = h[h_offset + en + en * h_dim1];
    if (l == en) {
	goto L270;
    }
    y = h[h_offset + na + na * h_dim1];
    w = h[h_offset + en + na * h_dim1] * h[h_offset + na + en * h_dim1];
    if (l == na) {
	goto L280;
    }
    if (itn == 0) {
...
/* L120: */
	h[h_offset + i__ + i__ * h_dim1] -= x;
    }

    s = fabs(h[h_offset + en + na * h_dim1]) + fabs(h[h_offset + na + enm2 * 
	    h_dim1]);
    x = s * .75;
    y = x;
    w = s * -.4375 * s;
...
/*                FOR M=EN-2 STEP -1 UNTIL L DO -- .......... */
    ...
    for (mm = l; mm <= i__1; ++mm) {
	m = enm2 + l - mm;
	zz = h[h_offset + m + m * h_dim1];
	r__ = x - zz;
	s = y - zz;
	p = (r__ * s - w) / h[h_offset + m + 1 + m * h_dim1] + h[h_offset + m + (m + 1) * 
		h_dim1];
	q = h[h_offset + m + 1 + (m + 1) * h_dim1] - zz - r__ - s;
	r__ = h[h_offset + m + 2 + (m + 1) * h_dim1];
	...
	tst1 = fabs(p) * (fabs(h[h_offset + m - 1 + (m - 1) * h_dim1]) + 
		fabs(zz) + fabs(h[h_offset + m + 1 + (m + 1) * h_dim1]));
	tst2 = tst1 + fabs(h[h_offset + m + (m - 1) * h_dim1]) * (fabs(q) 
		+ fabs(r__));
	if (tst2 == tst1) {
	    goto L150;
	}
...
L150:
	...
	h[h_offset + i__ + (i__ - 2) * h_dim1] = 0.;
	if (i__ == mp2) {
	    goto L160;
	}
	h[h_offset + i__ + (i__ - 3) * h_dim1] = 0.;
L160:
	;
    }
...
/*                COLUMNS M TO EN .......... */
	...
	p = h[h_offset + k + (k - 1) * h_dim1];
	q = h[h_offset + k + 1 + (k - 1) * h_dim1];
	r__ = 0.;
	if (notlas) {
	    r__ = h[h_offset + k + 2 + (k - 1) * h_dim1];
	}
...
L170:
	...
	h[h_offset + k + (k - 1) * h_dim1] = -s * x;
	goto L190;
L180:
	if (l != m) {
	    h[h_offset + k + (k - 1) * h_dim1] = -h[h_offset + k + (k - 1) * h_dim1];
	}
L190:
	p += s;
	x = p / s;
...
/*     .......... ROW MODIFICATION .......... */
	i__2 = n;
	for (j = k; j <= i__2; ++j) {
	    p = h[h_offset + k + j * h_dim1] + q * h[h_offset + k + 1 + j * h_dim1];
	    h[h_offset + k + j * h_dim1] -= p * x;
	    h[h_offset + k + 1 + j * h_dim1] -= p * y;
/* L200: */
	}
...
/*     .......... COLUMN MODIFICATION .......... */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    p = x * h[h_offset + i__ + k * h_dim1] + y * h[h_offset + i__ + (k + 1) * h_dim1];
	    h[h_offset + i__ + k * h_dim1] -= p;
	    h[h_offset + i__ + (k + 1) * h_dim1] -= p * q;
/* L210: */
	}
	goto L255;
L225:
/*     .......... ROW MODIFICATION .......... */
	i__2 = n;
	for (j = k; j <= i__2; ++j) {
	    p = h[h_offset + k + j * h_dim1] + q * h[h_offset + k + 1 + j * h_dim1] + r__ * h[h_offset + 
		    k + 2 + j * h_dim1];
	    h[h_offset + k + j * h_dim1] -= p * x;
	    h[h_offset + k + 1 + j * h_dim1] -= p * y;
	    h[h_offset + k + 2 + j * h_dim1] -= p * zz;
/* L230: */
	}
...
/*     .......... COLUMN MODIFICATION .......... */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    p = x * h[h_offset + i__ + k * h_dim1] + y * h[h_offset + i__ + (k + 1) * h_dim1] + 
		    zz * h[h_offset + i__ + (k + 2) * h_dim1];
	    h[h_offset + i__ + k * h_dim1] -= p;
	    h[h_offset + i__ + (k + 1) * h_dim1] -= p * q;
	    h[h_offset + i__ + (k + 2) * h_dim1] -= p * r__;
/* L240: */
	}
L255:
...
/* ----------------------------------------------------------------------- */
/* Subroutine */ int 
hqr2(integer nm, integer n, integer *low, integer *igh, doublereal *h, doublereal *wr, doublereal *wi, doublereal *z__, integer *ierr)
{
    /* System generated locals */
    integer h_offset, z_dim1, z_offset, i__1, i__2, i__3;
    ...
    doublereal norm;
    integer i__, j, k, l = 0, m = 0;
    doublereal p = 0, q, r__ = 0, s = 0, t, w, x, y;
    integer na, ii, en, jj;
    doublereal ra, sa;
    integer ll, mm, nn;
    doublereal vi, vr, zz;
...
/*                AND COMPUTE MATRIX NORM .......... */
    for (i__ = 1; i__ <= n; ++i__) {
	for (j = k; j <= n; ++j) {
	    norm += fabs(h[h_offset + i__ + j * nm]);
	}

	k = i__;
	if (i__ < *low || i__ > *igh) {
            wr[i__] = h[h_offset + i__ + i__ * nm];
            wi[i__] = 0.;
        }
    }
...
L70:
	...
	s = fabs(h[h_offset + l - 1 + (l - 1) * nm]) + fabs(h[h_offset + l 
		+ l * nm]);
	if (s == 0.) {
	    s = norm;
	}
	tst1 = s;
	tst2 = tst1 + fabs(h[h_offset + l + (l - 1) * nm]);
	if (tst2 == tst1) {
	    goto L100;
	}
/* L80: */
...
/*     .......... FORM SHIFT .......... */
L100:
    x = h[h_offset + en + en * nm];
    if (l == en) {
	goto L270;
    }
    y = h[h_offset + na + na * nm];
    w = h[h_offset + en + na * nm] * h[h_offset + na + en * nm];
    if (l == na) {
	goto L280;
    }
    if (itn == 0) {
...
/*     .......... FORM EXCEPTIONAL SHIFT .......... */
    ...
    for (i__ = *low; i__ <= i__1; ++i__) {
/* L120: */
	h[h_offset + i__ + i__ * nm] -= x;
    }

    s = fabs(h[h_offset + en + na * nm]) + fabs(h[h_offset + na + enm2 * nm]);
    x = s * .75;
    y = x;
    w = s * -.4375 * s;
L130:
...
/*                FOR M=EN-2 STEP -1 UNTIL L DO -- .......... */
    ...
    for (mm = l; mm <= i__1; ++mm) {
	m = enm2 + l - mm;
	zz = h[h_offset + m + m * nm];
	r__ = x - zz;
	s = y - zz;
	p = (r__ * s - w) / h[h_offset + m + 1 + m * nm] + h[h_offset + m + (m + 1) * 
		nm];
	q = h[h_offset + m + 1 + (m + 1) * nm] - zz - r__ - s;
	r__ = h[h_offset + m + 2 + (m + 1) * nm];
	s = fabs(p) + fabs(q) + fabs(r__);
	p /= s;
	q /= s;
	r__ /= s;
	...
	tst1 = fabs(p) * (fabs(h[h_offset + m - 1 + (m - 1) * nm]) + 
		fabs(zz) + fabs(h[h_offset + m + 1 + (m + 1) * nm]));
	tst2 = tst1 + fabs(h[h_offset + m + (m - 1) * nm]) * (fabs(q) + fabs(r__));
	if (tst2 == tst1) {
	    goto L150;
	}
/* L140: */
...
L150:
    ...
    i__1 = en;
    for (i__ = mp2; i__ <= i__1; ++i__) {
	h[h_offset + i__ + (i__ - 2) * nm] = 0.;
	if (i__ == mp2) {
	    goto L160;
	}
	h[h_offset + i__ + (i__ - 3) * nm] = 0.;
L160:
	;
    }
/*     .......... DOUBLE QR STEP INVOLVING ROWS L TO EN AND */
/*                COLUMNS M TO EN .......... */
	...
	p = h[h_offset + k + (k - 1) * nm];
	q = h[h_offset + k + 1 + (k - 1) * nm];
	r__ = 0.;
	if (notlas) {
	    r__ = h[h_offset + k + 2 + (k - 1) * nm];
	}
	x = fabs(p) + fabs(q) + fabs(r__);
	if (x == 0.) {
	    goto L260;
	...
	r__ /= x;
L170:
	s = d_sign(sqrt(p * p + q * q + r__ * r__), p);
	if (k == m) {
	    goto L180;
	}
	h[h_offset + k + (k - 1) * nm] = -s * x;
	goto L190;
L180:
	if (l != m) {
	    h[h_offset + k + (k - 1) * nm] = -h[h_offset + k + (k - 1) * nm];
	}
L190:
	p += s;
	x = p / s;
...
/*     .......... ROW MODIFICATION .......... */
	i__2 = n;
	for (j = k; j <= i__2; ++j) {
	    p = h[h_offset + k + j * nm] + q * h[h_offset + k + 1 + j * nm];
	    h[h_offset + k + j * nm] -= p * x;
	    h[h_offset + k + 1 + j * nm] -= p * y;
/* L200: */
	}
...
/*     .......... COLUMN MODIFICATION .......... */
	i__2 = j;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    p = x * h[h_offset + i__ + k * nm] + y * h[h_offset + i__ + (k + 1) * nm];
	    h[h_offset + i__ + k * nm] -= p;
	    h[h_offset + i__ + (k + 1) * nm] -= p * q;
/* L210: */
	}
...
/*     .......... ROW MODIFICATION .......... */
	i__2 = n;
	for (j = k; j <= i__2; ++j) {
	    p = h[h_offset + k + j * nm] + q * h[h_offset + k + 1 + j * nm] + r__ * h[h_offset + 
		    k + 2 + j * nm];
	    h[h_offset + k + j * nm] -= p * x;
	    h[h_offset + k + 1 + j * nm] -= p * y;
	    h[h_offset + k + 2 + j * nm] -= p * zz;
/* L230: */
	}
...
/*     .......... COLUMN MODIFICATION .......... */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    p = x * h[h_offset + i__ + k * nm] + y * h[h_offset + i__ + (k + 1) * nm] + 
		    zz * h[h_offset + i__ + (k + 2) * nm];
	    h[h_offset + i__ + k * nm] -= p;
	    h[h_offset + i__ + (k + 1) * nm] -= p * q;
	    h[h_offset + i__ + (k + 2) * nm] -= p * r__;
/* L240: */
	}
/*     .......... ACCUMULATE TRANSFORMATIONS .......... */
	i__2 = *igh;
...
/*     .......... ONE ROOT FOUND .......... */
L270:
    h[h_offset + en + en * nm] = x + t;
    wr[en] = h[h_offset + en + en * nm];
    wi[en] = 0.;
    en = na;
    goto L60;
/*     .......... TWO ROOTS FOUND .......... */
...
/*     .......... ROW MODIFICATION .......... */
    i__1 = n;
    for (j = na; j <= i__1; ++j) {
	zz = h[h_offset + na + j * nm];
	h[h_offset + na + j * nm] = q * zz + p * h[h_offset + en + j * nm];
	h[h_offset + en + j * nm] = q * h[h_offset + en + j * nm] - p * zz;
/* L290: */
    }
/*     .......... COLUMN MODIFICATION .......... */
    i__1 = en;
    for (i__ = 1; i__ <= i__1; ++i__) {
	zz = h[h_offset + i__ + na * nm];
	h[h_offset + i__ + na * nm] = q * zz + p * h[h_offset + i__ + en * nm];
	h[h_offset + i__ + en * nm] = q * h[h_offset + i__ + en * nm] - p * zz;
/* L300: */
    }
/*     .......... ACCUMULATE TRANSFORMATIONS .......... */
    i__1 = *igh;
...
/*     .......... FOR I=EN-1 STEP -1 UNTIL 1 DO -- .......... */
	...
	for (ii = 1; ii <= i__2; ++ii) {
	    i__ = en - ii;
	    w = h[h_offset + i__ + i__ * nm] - p;
	    r__ = 0.;

	    i__3 = en;
	    for (j = m; j <= i__3; ++j) {
/* L610: */
		r__ += h[h_offset + i__ + j * nm] * h[h_offset + j + en * nm];
	    }
...
L635:
	    h[h_offset + i__ + en * nm] = -r__ / t;
	    goto L680;
/*     .......... SOLVE REAL EQUATIONS .......... */
L640:
	    x = h[h_offset + i__ + (i__ + 1) * nm];
	    y = h[h_offset + i__ + 1 + i__ * nm];
	    q = (wr[i__] - p) * (wr[i__] - p) + wi[i__] * wi[i__];
	    t = (x * s - zz * r__) / q;
	    h[h_offset + i__ + en * nm] = t;
	    if (fabs(x) <= fabs(zz)) {
		goto L650;
	    }
	    h[h_offset + i__ + 1 + en * nm] = (-r__ - w * t) / x;
	    goto L680;
L650:
	    h[h_offset + i__ + 1 + en * nm] = (-s - y * t) / zz;

/*     .......... OVERFLOW CONTROL .......... */
L680:
	    t = fabs(h[h_offset + i__ + en * nm]);
	    if (t == 0.) {
		goto L700;
	    }
	    tst1 = t;
...
/*                EIGENVECTOR MATRIX IS TRIANGULAR .......... */
	if (fabs(h[h_offset + en + na * nm]) <= (d__2 = h[h_offset + na + en *
		 nm], fabs(d__2))) {
	    goto L720;
	}
	h[h_offset + na + na * nm] = q / h[h_offset + en + na * nm];
	h[h_offset + na + en * nm] = -(h[h_offset + en + en * nm] - p) / h[h_offset + en + na * 
		nm];
	goto L730;
L720:
	d__1 = -h[h_offset + na + en * nm];
	d__2 = h[h_offset + na + na * nm] - p;
        {
            doublereal c_b81 = 0.;
            cdiv(&c_b81, &d__1, &d__2, &q, &h[h_offset + na + na * nm], &h[h_offset + na + en *
                                                                       nm]);
        }
L730:
	h[h_offset + en + na * nm] = 0.;
	h[h_offset + en + en * nm] = 1.;
	enm2 = na - 1;
	if (enm2 == 0) {
	    goto L800;
...
/*     .......... FOR I=EN-2 STEP -1 UNTIL 1 DO -- .......... */
	...
	for (ii = 1; ii <= i__2; ++ii) {
	    ...
	    i__3 = en;
	    for (j = m; j <= i__3; ++j) {
		ra += h[h_offset + i__ + j * nm] * h[h_offset + j + na * nm];
		sa += h[h_offset + i__ + j * nm] * h[h_offset + j + en * nm];
/* L760: */
	    }
...
L770:
	    ...
	    d__1 = -ra;
	    d__2 = -sa;
	    cdiv(&d__1, &d__2, &w, &q, &h[h_offset + i__ + na * nm], &h[h_offset + i__ + 
		    en * nm]);
	    goto L790;
...
L784:
	    ...
	    d__2 = x * s - zz * sa - q * ra;
	    cdiv(&d__1, &d__2, &vr, &vi, &h[h_offset + i__ + na * nm], &h[h_offset + i__ + 
		    en * nm]);
	    if (fabs(x) <= fabs(zz) + fabs(q)) {
		goto L785;
	    }
	    h[h_offset + i__ + 1 + na * nm] = (-ra - w * h[h_offset + i__ + na * nm] + 
		    q * h[h_offset + i__ + en * nm]) / x;
	    h[h_offset + i__ + 1 + en * nm] = (-sa - w * h[h_offset + i__ + en * nm] - 
		    q * h[h_offset + i__ + na * nm]) / x;
	    goto L790;
L785:
	    d__1 = -r__ - y * h[h_offset + i__ + na * nm];
	    d__2 = -s - y * h[h_offset + i__ + en * nm];
	    cdiv(&d__1, &d__2, &zz, &q, &h[h_offset + i__ + 1 + na * nm], &h[h_offset + 
		    i__ + 1 + en * nm]);

/*     .......... OVERFLOW CONTROL .......... */
L790:
/* Computing MAX */
	    d__3 = fabs(h[h_offset + i__ + na * nm]), d__4 = (d__2 = 
		    h[h_offset + i__ + en * nm], fabs(d__2));
	    t = max(d__3,d__4);
	    if (t == 0.) {
		goto L795;
	    }
...
/*                VECTORS OF ISOLATED ROOTS .......... */
	...
	for (j = i__; j <= i__2; ++j) {
/* L820: */
	    z__[i__ + j * z_dim1] = h[h_offset + i__ + j * nm];
	}
...
/*                FOR J=N STEP -1 UNTIL LOW DO -- .......... */
	...
	for (i__ = *low; i__ <= i__2; ++i__) {
	    ...
	    for (k = *low; k <= i__3; ++k) {
/* L860: */
		zz += z__[i__ + k * z_dim1] * h[h_offset + k + j * nm];
	    }
...
L190:
	...
	for (j = k; j <= i__2; ++j) {
	    ...
	    c__ += fabs(a[j + i__ * a_dim1]);
	    r__ += fabs(a[i__ + j * a_dim1]);
L200:
	    ;
	}
/*     .......... GUARD AGAINST ZERO C OR R DUE TO UNDERFLOW .........
. */
	if (c__ == 0 || r__ == 0 || c__!=c__ || r__!=r__) {
            /* can be NANs too */
	    goto L270;
        }
	g = r__ / radix;
...
/*     .......... FOR L=N-1 STEP -1 UNTIL K+1 DO -- .......... */
	...
	for (lb = 1; lb <= i__2; ++lb) {
	    ...
	    l1 = l + 1;
/*     .......... ZERO A(L+1,K) .......... */
	    s = fabs(a[l + k * a_dim1]) + (d__2 = a[l1 + k * 
		    a_dim1], fabs(d__2));
	    if (s == 0.) {
		goto L150;
	    }
...
/*     .......... ZERO B(L+1,L) .......... */
	    s = fabs(b[l1 + l1 * b_dim1]) + (d__2 = b[l1 + l * 
		    b_dim1], fabs(d__2));
	    if (s == 0.) {
		goto L150;
	    }
...
/*     .......... COMPUTE EPSA,EPSB .......... */
	...
	ani = 0.;
	if (i__ != 1) {
	    ani = fabs(a[i__ + (i__ - 1) * a_dim1]);
	}
	bni = 0.;

	i__2 = n;
	for (j = i__; j <= i__2; ++j) {
	    ani += fabs(a[i__ + j * a_dim1]);
	    bni += fabs(b[i__ + j * b_dim1]);
/* L20: */
	}
...
L100:
    ...
    b[l + l * b_dim1] = 0.;
    s = fabs(a[l + l * a_dim1]) + (d__2 = a[l1 + l * a_dim1], 
	    fabs(d__2));
    u1 = a[l + l * a_dim1] / s;
    u2 = a[l1 + l * a_dim1] / s;
    d__1 = sqrt(u1 * u1 + u2 * u2);
...
/*     .......... DETERMINE SINGLE SHIFT ZEROTH COLUMN OF A .......... */
    ...
    sh = -t + r__;
    s = -t - r__;
    if (fabs(s - a44) < (d__2 = sh - a44, fabs(d__2))) {
	sh = s;
    }
/*     .......... LOOK FOR TWO CONSECUTIVE SMALL */
...
/*                FOR L=EN-2 STEP -1 UNTIL LD DO -- .......... */
	...
	l1 = l + 1;
	t = a[l + l * a_dim1];
	if (fabs(b[l + l * b_dim1]) > epsb) {
	    t -= sh * b[l + l * b_dim1];
	}
	if (fabs(a[l + lm1 * a_dim1]) <= (d__2 = t / a[l1 + l * 
...
L220:
	s = fabs(b[k2 + k2 * b_dim1]) + (d__2 = b[k2 + k1 * 
...
/*     .......... ZERO B(K+1,K) .......... */
L240:
	s = fabs(b[k1 + k1 * b_dim1]) + (d__2 = b[k1 + k * 
		b_dim1], fabs(d__2));
	if (s == 0.) {
	    goto L260;
	}
...
L410:
	...
	beta[en] = fabs(b[en + en * b_dim1]);
	alfi[en] = 0.;
	goto L510;
/*     .......... 2-BY-2 BLOCK .......... */
L420:
	if (fabs(b[na + na * b_dim1]) <= epsb) {
	    goto L455;
	}
	if (fabs(b[en + en * b_dim1]) > epsb) {
	    goto L430;
	}
	a1 = a[en + en * a_dim1];
	a2 = a[en + na * a_dim1];
...
ndsvd(doublereal *x, integer *ldx, integer *n, integer *p, doublereal *s, doublereal *e, doublereal *u, integer *ldu, doublereal *v, integer *ldv, doublereal *work, integer *job, integer *info, integer *maxitr, doublereal *tol, integer *idbg, integer *ifull, integer *kount, integer *kount1, integer *kount2, integer *skip, integer *limshf, doublereal *maxsin, integer *iidir)
{
    ...
    doublereal oldcs;
    integer oldll, iisub;
    doublereal shift, oldsn = 0, sigmn;
    integer minnp, maxit;
    doublereal sminl;

    doublereal sigmx;
    logical wantu, wantv;
    doublereal gg = 0, lambda;
    integer oldacc;
    doublereal cs;
    integer ll, mm;
    doublereal sm;
...
/*       absolute accuracy desired */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
/* Computing MAX */
	    d__3 = smax, d__4 = fabs(s[i__]), d__3 = max(d__3,
...
/*     compute minimum s(i) and max of all s(i),e(i) */
    if (*tol <= (double)0. && fabs(s[m]) <= thresh) {
	s[m] = (double)0.;
    }
    smax = fabs(s[m]);
    smin = smax;
...
/*     reset convergence threshold if starting new part of matrix */
	...
	if (*tol <= (double)0. && fabs(s[ll]) <= thresh) {
	    s[ll] = (double)0.;
	}
	if (fabs(e[ll]) <= thresh) {
	    goto L1002;
...
/*     choose shift direction if working on entirely new submatrix */
    if (ll > oldm || m < oldll) {
	if ((fabs(s[ll]) >= (d__2 = s[m], fabs(d__2)) && *iidir == 0) || *iidir == 1) {
...
/*         apply test on bottom 2 by 2 only */
	    if (fabs(e[m - 1]) <= *tol * (d__2 = s[m], fabs(d__2)
		    )) {
...
/*         apply test on top 2 by 2 only */
	    if (fabs(e[ll]) <= *tol * (d__2 = s[ll], fabs(d__2)))
		     {
...
/*             test for negligibility satisfied */
		    ...
		    lambda = fabs(s[lll]) * (lambda / (lambda + 
...
/*       if idir=1, chase bulge from top to bottom */
	if (idir == 1) {
	    ...
	    f = (fabs(s[ll]) - shift) * (d_sign(c_b170, s[ll])
		     + shift / s[ll]);
	    g = e[ll];
	    i__1 = m - 1;
	    for (k = ll; k <= i__1; ++k) {
...
/*       (idir=2, so chase bulge from bottom to top) */
	    ...
	    f = (fabs(s[m]) - shift) * (d_sign(c_b170, s[m]) 
		    + shift / s[m]);
	    g = e[m - 1];
	    i__1 = ll + 1;
	    for (k = m; k >= i__1; --k) {
...
sig22(doublereal *a, doublereal *b, doublereal *c__, doublereal *sigmin, doublereal *sigmax, doublereal *snr, doublereal *csr, doublereal *snl, doublereal *csl)
{
    ...
    doublereal absa, absb, absc, acmn, acmx, sgna, sgnb, sgnc, cosl, 
	    sinl, cosr, temp, sinr, temp1 = 0, temp2 = 0, temp3, sgnmn, sgnmx, ac, ca;
    integer ia;
    doublereal absbac;
    integer ib;
    doublereal as = 0, at = 0, au = 0;
...
L40:
    ...
    if (fabs(a[l + (l - 1) * a_dim1]) <= *eps * ((d__2 = a[l - 
...
/* L60: */
    ...
    s = fabs(a[nu + (nu - 1) * a_dim1]) + (d__2 = a[nu - 1 + (
...
L80:
    ...
    if (fabs(a[nl + (nl - 1) * a_dim1]) * (fabs(q) + fabs(r__)) <=
...
L40:
    r__ = 0.;
L50:
    if (fabs(x + z__) >= (d__2 = x + r__, fabs(d__2))) {
	z__ = r__;
    }
    y = y - x - z__;
    x = -z__;
...
L60:
    ...
    i__1 = *l + 2;
    qrstep(&a[a_offset], &v[v_offset], &p, &q, &r__, l, &i__1, n, na, nv);
    if (fabs(a[*l + 2 + (*l + 1) * a_dim1]) > *eps * ((d__2 = a[
...
L90:
    ...
    r__ /= s;
    qrstep(&a[a_offset], &v[v_offset], &p, &q, &r__, l, &m, n, na, nv);
    if (fabs(a[m - 1 + (m - 2) * a_dim1]) > *eps * ((d__2 = a[m 
