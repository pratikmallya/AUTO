...
/*     ---------- ---- */
/* Subroutine */ int 
fnho(const iap_type *iap, const rap_type *rap, integer ndim, doublereal *u, const doublereal *uold, const integer *icp, doublereal *par, integer ijac, doublereal *f, doublereal *dfdu, doublereal *dfdp)
{
...
/* Generate the function. */
  ...
  if (blhom_1.istart >= 0 && blhom_1.itwist == 1)
  ...
  for (i = 0; i < ndim; ++i) {
    uu = u[i];
    u[i] = uu - ep;
    ffho(iap, rap, ndim, u, uold, icp, par, &ARRAY2D(dfdu, 0, i), ndm, dfu);
    u[i] = uu + ep;
    ffho(iap, rap, ndim, u, uold, icp, par, f, ndm, dfu);
    u[i] = uu;
    for (j = 0; j < ndim; ++j) {
      ARRAY2D(dfdu, j, i) = (f[j] - ARRAY2D(dfdu, j, i)) / (ep * 2);
    }
  }
  ...
  for (i = 0; i < iap->nfpr; ++i) {
    par[icp[i]] += ep;
    ffho(iap, rap, ndim, u, uold, icp, par, &ARRAY2D(dfdp, 0, icp[i]), ndm, dfu);
    for (j = 0; j < ndim; ++j) {
      ARRAY2D(dfdp, j, icp[i]) = (ARRAY2D(dfdp, j, icp[i]) - f[j]) / ep;
    }
    par[icp[i]] -= ep;
  }
  if (dfu) free(dfu);
...
/*     ---------- ---- */
/* Subroutine */ int 
ffho(const iap_type *iap, const rap_type *rap, integer ndim, doublereal *u, const doublereal *uold, const integer *icp, doublereal *par, doublereal *f, integer ndm, doublereal *dfdu)
{
  /* System generated locals */
  integer dfdu_dim1;
  ...
  if (blhom_1.istart >= 0) {
    if (blhom_1.itwist == 0) {
      ...
      user.func(ndm, u, icp, par, 0, f, NULL, NULL);
    ...
    for (j = 0; j < ndim; j += ndm) {
      user.func(ndm, &u[j], icp, par, 0, &f[j], NULL, NULL);
    }      
  }
...
/* Generate the function. */
  ...
  fbho(iap, rap, ndim, par, icp, nbc, nbc0, u0, u1, f);
  ...
  for (i = 0; i < ndim; ++i) {
    uu[i] = u0[i] - ep;
    fbho(iap, rap, ndim, par, icp, nbc, nbc0, uu, u1, ff1);
    uu[i] = u0[i] + ep;
    fbho(iap, rap, ndim, par, icp, nbc, nbc0, uu, u1, ff2);
    uu[i] = u0[i];
    for (j = 0; j < nbc; ++j) {
  ...
  for (i = 0; i < ndim; ++i) {
    uu[i] = u1[i] - ep;
    fbho(iap, rap, ndim, par, icp, nbc, nbc0, u0, uu, ff1);
    uu[i] = u1[i] + ep;
    fbho(iap, rap, ndim, par, icp, nbc, nbc0, u0, uu, ff2);
    uu[i] = u1[i];
    for (j = 0; j < nbc; ++j) {
  ...
  for (i = 0; i < nfpr; ++i) {
    par[icp[i]] += ep;
    fbho(iap, rap, ndim, par, icp, nbc, nbc0, u0, u1, ff2);
    for (j = 0; j < nbc; ++j) {
...
static int fbho_regular(const iap_type *iap, const rap_type *rap, integer ndim, doublereal *par,
                        const integer *icp, const doublereal *u0,
                        const doublereal *u1, doublereal *fb,
                        doublereal *xequib1, doublereal *xequib2)
{
    /* Local variables */
  ...
  if (blhom_1.nrev == 0) {
    prjctn(iap, rap, bound, xequib2, icp, par, 1, 2, 1, ndm);
  ...
  if (blhom_1.nfixed > 0 || blhom_1.iequib == 2) {
    ...
    if (blhom_1.nfixed > 0) {
      ...
      eigho(iap, rap, 2, rr[0], ri[0], vr[0], xequib1, icp, par, ndm);
      if (blhom_1.iequib < 0) {
        eigho(iap, rap, 2, rr[1], ri[1], vr[1], xequib2, icp, par, ndm);
      }
      for (i = 0; i < blhom_1.nfixed; ++i) {
        if (blhmp_1.ifixed[i] > 10 && vt[0] == NULL) {
          vt[0] = &bound[3 * ndm];
          eigho(iap, rap, 1, rr[0], ri[0], vt[0], xequib1, icp, par, ndm);
          if (blhom_1.iequib < 0) {
            vt[1] = &bound[4 * ndm];
            eigho(iap, rap, 1, rr[1], ri[1], vt[1], xequib2, icp, par, ndm);
          }
        }
  ...
  if (blhom_1.iequib != 0 && blhom_1.iequib != -1) {
    user.func(ndm, xequib1, icp, par, 0, &fb[jb], NULL, NULL);
    jb += ndm;
    /*        *NDM extra initial conditions for the equilibrium if IEQUIB=-2 */
    if (blhom_1.iequib == -2) {
      user.func(ndm, xequib2, icp, par, 0, &fb[jb], NULL, NULL);
      jb += ndm;
    }
  }
  ...
  if (blhom_1.itwist == 1 && blhom_1.istart >= 0) {
    ...
    prjctn(iap, rap, bound, xequib1, icp, par, 1, 1, 2, ndm);
    for (i = ndm - blhom_1.nunstab; i < ndm; ++i) {
      fb[jb++] = fbho_ddot(&u0[ndm], bound[i], ndm);
    }
    /*           *-orthogonal to the stable directions of A  at t=1 */
    prjctn(iap, rap, bound, xequib2, icp, par, -1, 2, 2, ndm);
    for (i = 0; i < blhom_1.nstab; ++i) {
    ...
    if (umax == NULL) {
      umax = (doublereal *)malloc(sizeof(doublereal)*ndim);
      for (i = 0; i < ndim; ++i) {
        umax[i] = u1[i];
      }
    }
    for (k = 0; k < ndim/ndm-1; ++k) {
      fb[jb++] += fbho_ddotsub(&par[NPARX - ndm], &u1[k * ndm], &umax[k * ndm], ndm);
    }
  }
  free_dmatrix(bound);
  return jb;
...
static int fbho_homotopy(const iap_type *iap, const rap_type *rap, integer ndm, doublereal *par,
                         const integer *icp, const doublereal *u0,
                         const doublereal *u1, doublereal *fb,
                         doublereal *xequib1, doublereal *xequib2)
{
  ...
  vr = dmatrix(ndm, ndm);
  eigho(iap, rap, 2, rr, ri, vr, xequib1, icp, par, ndm);
  jb = ndm;
  ...
  vt = dmatrix(ndm, ndm);
  eigho(iap, rap, 1, rr, ri, vt, xequib2, icp, par, ndm);
  free(rr);
  free(ri);
  for (i = ndm - blhom_1.nunstab; i < ndm; ++i) {
    ++kp;
...
/*     ---------- ---- */
/* Subroutine */ int 
fbho(const iap_type *iap, const rap_type *rap, integer ndim, doublereal *par, const integer *icp, integer nbc, integer nbc0, const doublereal *u0, const doublereal *u1, doublereal *fb)
{
  integer i, jb, nbcn;
  integer ndm = iap->ndm;
  ...
  if (global_rotations.irtn != 0) {
    xequib2 = malloc(ndim * sizeof(doublereal));
    for (i = 0; i < ndim; ++i) {
      xequib2[i] = xequib1[i];
      if (global_rotations.nrtn[i] != 0) {
	if (blhom_1.istart < 0)
	  par[18] = -blhom_1.istart * pi(2.0);
	xequib2[i] += par[18] * global_rotations.nrtn[i];
      }
    }
  } else if (blhom_1.iequib >= 0) {
    xequib2 = &par[11];
  } else {
    xequib2 = &par[ndm + 11];
  }
  ...
  if (blhom_1.istart != 3) {
    jb = fbho_regular(iap, rap, ndim, par, icp, u0, u1, fb, xequib1, xequib2);
  } else {
    /*     **Starting Solutions using Homotopy** */
    jb = fbho_homotopy(iap, rap, ndm, par, icp, u0, u1, fb, xequib1, xequib2);
  }
  /*      write(9,*) NBCN,NBC */
  /* *user defined extra boundary conditions */
  nbcn = nbc - jb;
  if (nbcn > 0) {
    user.bcnd(ndim, par, icp, nbcn, u0, u1, 0, &fb[jb], NULL);
  } else if (nbcn < 0) {
...
/*     ---------- ---- */
/* Subroutine */ int 
icho(const iap_type *iap, const rap_type *rap, integer ndim, doublereal *par, const integer *icp, integer nint, doublereal *u, const doublereal *uold, const doublereal *udot, const doublereal *upold, doublereal *f, integer ijac, doublereal *dint)
{
  /* System generated locals */
  integer dint_dim1;
  ...
  doublereal rtmp;
  integer i, j;
  doublereal ep, uu, umx;
  integer nnt0 = iap->nnt0;
...
/* Generates integral conditions for homoclinic bifurcation analysis */
  ...
  if (ijac == 0) {
    ...
    for (i = 0; i < ndim; ++i) {
      ...
      fiho(iap, rap, ndim, par, icp, nint, nnt0, u, uold, udot, upold, f);
      u[i] = uu + ep;
      fiho(iap, rap, ndim, par, icp, nint, nnt0, u, uold, udot, upold, &ARRAY2D(dint, 0, i));
      u[i] = uu;
      for (j = 0; j < nint; ++j) {
        ARRAY2D(dint, j, i) = (ARRAY2D(dint, j, i) - f[j]) / (ep * 2);
      }
    }
...
/* Generate the function. */
    ...
    if (ijac > 1) for (i = 0; i < nfpr; ++i) {
      par[icp[i]] += ep;
      fiho(iap, rap, ndim, par, icp, nint, nnt0, u, uold, udot, upold, &ARRAY2D(dint, 0, ndim + icp[i]));
      for (j = 0; j < nint; ++j) {
        ARRAY2D(dint, j, ndim + icp[i]) = (ARRAY2D(dint, j, ndim + icp[i]) - f[j]) / ep;
      }
      par[icp[i]] -= ep;
    }
  }
...
/*     ---------- ---- */
/* Subroutine */ int 
fiho(const iap_type *iap, const rap_type *rap, integer ndim, doublereal *par, const integer *icp, integer nint, integer nnt0, doublereal *u, const doublereal *uold, const doublereal *udot, const doublereal *upold, doublereal *fi)
{
  /* Local variables */
...
/* Integral phase condition for homoclinic orbit */
  ...
  if (blhom_1.nrev == 0 && blhom_1.istart >= 0) {
    ...
    if (blhom_1.nrev == 0 && blhom_1.itwist == 1 && blhom_1.istart >= 0) {
      dum = 0.;
      for (i = ndm; i < ndm + ndm; ++i) {
        dum += uold[i] * (u[i] - uold[i]);
      }
      fi[jb++] = dum;
    }
  }
  ...
  if (jb < nint) {
    user.icnd(ndm, par, icp, nint, u, uold, udot, upold, 0, &fi[jb], NULL);
  }
  return 0;
} /* fiho_ */
...
/*     ---------- ---- */
/* Subroutine */ int 
inho(iap_type *iap, integer **icp_pointer, doublereal *par)
{

    /* Local variables */
  integer ndim, nint, nuzr, i, nfree, icorr, nbc, ndm, irs, isw, *icp;
...
/* set various constants */
  ...
  /*        n-homoclinic branch switching */
    ...
    icp = realloc(icp, nfree * sizeof(*icp));
  ...
  /* nondegeneracy parameter of the adjoint */
    ...
    icp = realloc(icp, (nfree + 1) * sizeof(*icp));
    icp[nfree] = 9;
    ++nfree;
    par[9] = 0.;
    ndim = ndm * 2;
  ...
  if (blhom_1.iequib != 0 && blhom_1.iequib != -1) {
    icp = realloc(icp, (nfree + ndm) * sizeof(*icp));
    for (i = 0; i < ndm; ++i) {
      icp[nfree + i] = i + 11;
    }
  }
  ...
  if (blhom_1.iequib == -2) {
    icp = realloc(icp, (nfree + 2 * ndm) * sizeof(*icp));
    for (i = 0; i < ndm; ++i) {
...
/*     Called by PREHO */
{
  ...
  integer i2, j2[3], k2, addphase;
  doublereal tt[3];
  doublereal upsmax, phasediff;
...
/* Find maximum from the equilibrium */
  ...
  if (global_rotations.irtn != 0) {
    ...
    upsmax = 0;
    for (i = 0; i < ndm; ++i)
      if (global_rotations.nrtn[i]) break;
    for (j = 0; j <= *ntsr; ++j) {
      double d1 = fabs(ups[j][i] - par[i + 11]);
      upsi = fabs(ups[j][i] - (par[i + 11] + par[18] * global_rotations.nrtn[i]));
      if (d1 < upsi) upsi = d1;
      if (upsi > upsmax) {
	upsmax = upsi;
	jmax = j;
  ...
  user.func(ndm, ups[jmax], icp, par, 0, &par[NPARX-ndm], NULL, NULL);
...
/*     so that we can subsequently overwrite the beginning. */
  ...
  for (l = (*ntsr << 1) - 1; l >= *ntsr - 1; --l) {
	...
	if (global_rotations.irtn != 0) {
	  phasediff = 0;
	  if (addphase)
	    phasediff = par[18] * global_rotations.nrtn[i % ndm];
	}
        ups[l][i + ndm] = ups[j][i] + phasediff;
        udotps[l][i + ndm] = udotps[j][i] + phasediff;
        ups[l][i] = ups[j][i];
	  ...
	  if (global_rotations.irtn != 0) {
	    phasediff = par[18] * global_rotations.nrtn[i % ndm] *
          ...
          ups[l + jmax + 1][i + ndim - ndm] = ups[j][i] + phasediff;
          udotps[l + jmax + 1][i + ndim - ndm] = udotps[j][i] + phasediff;
        }
      }
    }
  }
...
/*     Remesh last part : */
    ...
    intpho(iap, rap, ndm, ncolrs, tt[2], t[2] - tt[2], ndx, ups, udotps, tm[j - 1],
           dtm[j - 1], ndim - ndm, ndim, j2[2] + jmax, j - 1);
...
/*     where n>=3 and NDIM=(n+1)*NDM: (NDIM/NDM)-3 times. */
    ...
    for (k2 = ndm; k2 < ndim - ndm * 2; k2 += ndm) {
      for (k = ndm; k <= (ncolrs - 1) * ndim + ndm; k += ndim) {
        for (i = k; i < k + ndm; ++i) {
	  if (global_rotations.irtn != 0)
	    phasediff = par[18] * global_rotations.nrtn[i % ndm] * (k2/ndm);
          ups[j - 1][i + k2] = ups[j - 1][i] + phasediff;
          udotps[j - 1][i + k2] = udotps[j - 1][i] + phasediff;
        }
      }
    }
    ++j2[i2];
...
/*     Adjust end points */
  ...
  for (i = 0; i < ndm; ++i) {
    if (global_rotations.irtn != 0)
      phasediff = par[18] * global_rotations.nrtn[i];
    for (k2 = i; k2 < ndim; k2 += ndm) {
      ups[*ntsr][k2] = ups[*ntsr + 1][i + ndm] + phasediff * ((k2 - i)/ndm - 1);
      udotps[*ntsr][k2] = udotps[*ntsr + 1][i + ndm] + phasediff * ((k2 - i)/ndm - 1);
    }
    ups[*ntsr][i + ndim - ndm] = ups[0][i] + phasediff * -blhom_1.istart;
    udotps[*ntsr][i + ndim - ndm] = udotps[0][i] + phasediff * -blhom_1.istart;
  }
  
  /*     rotations: par[18] needs adjustment */
  if (global_rotations.irtn != 0)
    par[18] *= -blhom_1.istart;
...
/*     again once the branch switching is complete. */
{
  ...
  for (k = 0; k < ndm; ++k) {
    ups[*ntsr][k] = ups[*ntsr][k+(ncopy-1)*ndm];
    udotps[*ntsr][k] = udotps[*ntsr][k+(ncopy-1)*ndm];
  }
  for (k = ncopy - 1; k >= 0; --k) {
    for (j = *ntsr - 1; j >= 0; --j) {
      i = j + *ntsr * k;
  ...
  if (blhom_1.istart == 4) {
    ...
    if (blhom_1.iequib > 0) {
      ...
      for (j = 0; j <= *ntsr; ++j) {
        ...
        user.func(ndm, ui, icp, par, 0, f, &dum1, &dum2);
        upsi = 0.;
        for (i = 0; i < ndm; ++i) {
          upsi += f[i] * f[i];
        }
	...
	udotps[*ntsr][k] = udotps[0][k];
      ...
      if (global_rotations.irtn != 0) {
        for (j = 0; j <= *ntsr; ++j) {
          for (i = 0; i < ndm; ++i) {
            if (global_rotations.nrtn[i] != 0) {
              if (fabs((ups[j+1][i] - ups[j][i])/global_rotations.nrtn[i]) >
                  fabs(par[18]/2)) {
                ++j;
                goto out;
              }
      ...
      out:
        for (; j <= *ntsr; ++j) {
          for (i = 0; i < ncolrs * ndim; ++i) {
            if (global_rotations.nrtn[i % ndim] != 0) {
              ups[j][i] += par[18] * global_rotations.nrtn[i % ndim];
            }
          }        
        }
      }
...
{
  integer ndim, ncol, ntst, ncol1, i, j, k, k2;
  integer ip;
  integer kp;
  integer ndm;
  ...
  if(iap->irs > 0) {
    integer ndimrd;
    ndimrd = stpnbv(iap, rap, par, icp, ntsr, ncolrs, rlcur, rldot, ndxloc,
		    ups, udotps, upoldp, tm, dtm, nodir, thl, thu);
    /* Preprocess restart data */
    preho(iap, rap, par, icp, ndxloc, ntsr, &ndimrd, *ncolrs, ups, udotps, tm, dtm);
    return 0;
  }
  ...
  /* Initialize solution from user and parameters */
  stpnub(iap, rap, par, icp, ntsr, ncolrs, rlcur, rldot, ndxloc,
	 ups, udotps, upoldp, tm, dtm, nodir, thl, thu);
  ...
  user.pvls(ndm, ups, par);
  xequib = &par[11];
  eigho(iap, rap, 1, rr, ri, vt, xequib, icp, par, ndm);
  eigho(iap, rap, 2, rr, ri, vr, xequib, icp, par, ndm);
...
*/
  ...
  for (j = 0; j < ntst + 1; ++j) {
    ...
    for (i = 0; i < ncol1; ++i) {
      ...
      for (k = 0; k < ndim; ++k) {
	ups[j][k2 + k] = xequib[k] +
	  vr[blhom_1.nstab][k] * par[kp] * par[kp + 1] *
...
pvlsho(iap_type *iap, rap_type *rap, integer *icp, doublereal *dtm, integer ndxloc, doublereal **ups, integer *ndim, doublereal **p0, doublereal **p1, doublereal *par)
{
  ...
  /*      *Compute eigenvalues */
  bleig.ineig = 0;
  eigho(iap, rap, 2, bleig.rr[0], bleig.ri[0], bleig.v[0], &par[11], icp, par, ndm);
  if (blhom_1.iequib < 0) {
    eigho(iap, rap, 2, bleig.rr[1], bleig.ri[1], bleig.v[1], &par[11 + ndm], icp, par, ndm);
  }
  if (iid >= 3) {
  ...
  if (blhom_1.itwist == 1 && blhom_1.istart >= 0) {
    eigho(iap, rap, 1, bleig.rr[0], bleig.ri[0], bleig.vt[0], 
	  &par[11], icp, par, ndm);
    if (blhom_1.iequib < 0) {
      eigho(iap, rap, 1, bleig.rr[1], bleig.ri[1], bleig.vt[1],
  ...
  for (i = 0; i < blhom_1.npsi; ++i) {
    if (blhmp_1.ipsi[i] > 10 && bleig.ineig == 0) {
      eigho(iap, rap, 1, bleig.rr[0], bleig.ri[0], bleig.vt[0], &par[11], icp, par, ndm);
      if (blhom_1.iequib < 0) {
        eigho(iap, rap, 1, bleig.rr[1], bleig.ri[1], bleig.vt[1], &par[11 + ndm], icp, par, ndm);
      }
      bleig.ineig = 1;
    }
...
/* Local */
  ...
  user.func(ndm, pu0, icp, par, 0, f0, NULL, NULL);
  user.func(ndm, pu1, icp, par, 0, f1, NULL, NULL);
...
/*     ---------- ----- */
/* Subroutine */ int 
eigho(const iap_type *iap, const rap_type *rap, integer itrans, doublereal *rr, doublereal *ri, doublereal **vret, doublereal *xequib, const integer *icp, doublereal *par, integer ndm)
{
  /* System generated locals */
  integer dfdu_dim1;
...
/* Local */
  ...
  funi(iap, rap, ndm, xequib, NULL, icp, par, 1, f, dfdu, NULL);
  ...
  for (j = 0; j < ndm; ++j) {
    if (ri[j] > HMACHHO && j < ndm - 1) {
      for (i = 0; i < ndm; ++i) {
	vr[i][j] = zz[j][i];
	vi[i][j] = zz[j + 1][i];
      }
    } else if (ri[j] < -HMACHHO && j > 0) {
      for (i = 0; i < ndm; ++i) {
	vr[i][j] = zz[j - 1][i];
	vi[i][j] = -zz[j][i];
      }
...
/*     ---------- ------ */
/* Subroutine */ int 
prjctn(const iap_type *iap, const rap_type *rap, doublereal **bound, doublereal *xequib, const integer *icp, doublereal *par, integer imfd, integer is, integer itrans, integer ndm)
{
    /* Local variables */
  integer i, j, k;
...
/* called with the same values of IS and ITRANS. */
  fdum = (doublereal *)malloc(sizeof(doublereal)*ndm*(2*ndm + 3));
  a = fdum + ndm;
  v = fdum + ndm * (ndm + 1);

  funi(iap, rap, ndm, xequib, NULL, icp, par, 1, fdum, a, NULL);
  ...
  /* Compute basis V to put A in upper Hessenberg form */
  {
    /* This is here since I don't want to change the calling sequence of the
       BLAS routines. */
    integer tmp = 1;
    orthes(ndm, ndm, &tmp, &ndm, a, fdum);
    ortran(ndm, ndm, &tmp, &ndm, a, fdum, v);
  }
  ...
  eps = HMACHHO;
  {
    ...
    doublereal *eig = fdum + ndm * (2 * ndm + 1);
    hqr3lc(a, v, &ndm, &tmp, &ndm, &eps, eig, eig+ndm, type__, &ndm, &ndm, &imfd);
    free(type__);
  }
  ...
  if (cprev == NULL) {
    ...
    doublereal **d = &cnow[ndm];
    doublereal **dum1 = &cnow[2 * ndm];
    doublereal **dum2 = &cnow[3 * ndm];
